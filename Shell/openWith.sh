...
open() {
    case "$1" in
        ...
        *) rm -f /tmp/image* ; rm -f /tmp/para.txt ; python $HOME/Git/OneOffCodes/Python/paragraph_scraper.py --url "$1" > /dev/null 2>&1 ; python $HOME/Git/OneOffCodes/Python/image_scraper.py --url "$1" > /dev/null 2>&1; feh --scale-down /tmp/image* & urxvtc -geometry 50x2 -e speedread -w 375 /tmp/para.txt
