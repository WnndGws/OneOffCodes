...
case "$x" in
    summary) clear; rm -f /tmp/para* > /dev/null 2>&1; python $HOME/Git/OneOffCodes/Python/paragraph_scraper/paragraph_scraper.py --url "$1"; python $HOME/Git/OneOffCodes/Python/paragraph_scraper/article_summarise.py; gvim "+term speedread -w 350 /tmp/para_summarise.txt" /tmp/para_summarise.txt;;
    ...
    paragraph) clear; rm -f /tmp/para* > /dev/null 2>&1; python $HOME/Git/OneOffCodes/Python/paragraph_scraper/paragraph_scraper.py --url "$1"; gvim "+term speedread -w 350 /tmp/para.txt" /tmp/para.txt ;;
    *) "$x" "$1"
esac
